...
id = createdProd._id
   ...
   it("it should be ok with the product model", () => {
    expect(createdProd).to.keys("title", "price", "thumbnail", "descrip", "stock", "codigo","_id", "__v"  )
...
describe(`-PUT /api/productos/Listado/${id}`, () =>{
 ...
 it("Should return 200", async () => {
    const response = await request.put(`/api/productos/Listado/${id}`).send(proof)
...
describe("-GET /api/productos/Listado/:id", async () =>{
  const pro = "63471dbf288c73923ce99667"  
    it("it should return 200", async() => {
    const response= await  request.get(`/api/productos/Listado/${pro}`)
product = response.body
    expect(response.status).to.eql(200)

    })
   it("it should return the asked product", async() =>{

    expect(product._id).to.eql(pro)

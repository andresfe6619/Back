...
class Contenedor {
  ...
  async addProductFrom() {
    const data = JSON.parse(
      await fs.promises.readFile("./contenedor2.json", "utf-8")
    );
  ...
  async getAllCarts() {
    ...
    return data
      ? JSON.parse(data)
      : logger.error("El archivo no existe o esta vacio");
  }
  ...
  async addFromdb(Id) {
    const data = JSON.parse(
      await fs.promises.readFile("./contenedor.json", "utf-8")
    );
  ...
  async erase(Id) {
    const data = JSON.parse(
      await fs.promises.readFile("./carritoContenedor.json", "utf-8")
    );

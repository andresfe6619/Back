...
router.get("/consigna", (ctx: Context):void => {
    const arrayColors = []
    const phrase = ctx.request.url.searchParams.get('color')
    const phraseArray = phrase?.split(" ")
    const reversePhraseArray = phraseArray?.reverse()
    const joinReversePhraseArray = reversePhraseArray?.join(" ")
    arrayColors.push(joinReversePhraseArray)
    console.log( reversePhraseArray)
    ...
    ctx.response.body = `
        ...
        <body>   
        <h1 style="color:${arrayColors};">${arrayColors}</h1>   
     </body> 
     </html>  `
})
